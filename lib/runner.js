...
app.on('ready', function() {
  ...
  parent.respondTo('browser-initialize', function(opts, done) {
    ...
    win.on('closed', () => {
      parent.emit.apply(parent, ['page', 'closedBrowser'])
    })
    ...
    /**
     * Sets WebRTC IP Handling Policy.
     */

    if (options.webRTCPolicy) {
      win.webContents.setWebRTCIPHandlingPolicy(options.webRTCPolicy)
    }
    ...
    renderer.on('innerEvent', function(_sender /*, arguments, ... */) {
      parent.emit.apply(parent, ['innerEvent'].concat(sliced(arguments, 1)))
    })
